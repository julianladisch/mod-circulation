...
public class PageableFetcherTest {
  ...
  public void shouldProcessPages() {
    ...
    final var fetcher = spy(pageFetcher(100));
    final var pageProcessor = spy(dummyProcessor());

    final var voidResult = processPages(fetcher, pageSize, pageProcessor);
  ...
  public void shouldAbortFetchingWhenPageProcessorRaisedError() {
    ...
    final var pageSize = limit(10);
    final var fetcher = pageFetcher(50);
    final var pageProcessor = mock(PageProcessor.class);
  ...
  public void shouldAbortFetchingWhenFailedToFetch() {
    final var failureMessage = "Fetch failure";
    final var pageSize = limit(10);
    final var repository = mock(PageableFetcher.PageFetcher.class);
    final var pageProcessor = mock(PageProcessor.class);

    when(repository.fetch(any(), any(), any()))
      .thenReturn(completedFuture(failed(new ServerErrorFailure(failureMessage))));
  ...
  public void shouldAbortFetchingWhenRecordCountLimitIsReached() {
    ...
    final var pageFetcher = pageFetcher(recordLimit + 10);
    final PageProcessor<Integer> pageProcessor = spy(dummyProcessor());

    final var voidResult = new PageableFetcher<>(pageFetcher, pageSize, recordLimit)
  ...
  public void shouldNotAbortFetchingWhenRecordCountLimitIsReachedButAllPagesFetched() {
    ...
    final var voidResult = new PageableFetcher<>(pageFetcher(recordLimit), pageSize, recordLimit)
      .processPages(noQuery().value(), pageProcessor)
      .getNow(Result.failed(new ServerErrorFailure("Time out")));
  ...
  private <T> Result<Void> processPages(PageableFetcher.PageFetcher<T> pageFetcher,
    PageLimit pageLimit, PageProcessor<T> processor) {

    return new PageableFetcher<>(pageFetcher, pageLimit, 1000)
  ...
  private PageableFetcher.PageFetcher<Integer> pageFetcher(int totalRecords) {
    return new PageableFetcher.PageFetcher<Integer>() {
      @Override
      public CompletableFuture<Result<MultipleRecords<Integer>>> fetch(
